...
from .SOS import DYReLU
from .OCOR import OCOR
...
class DIIHead(BBoxHead):
    ...
    def __init__(self,
        ...
        # define sos module
        self.sos = DYReLU(inp=256, oup=256)

        # define ocor module
        self.ds_obj = nn.AdaptiveAvgPool2d((2, 2))
        self.ds_glb = nn.AdaptiveAvgPool2d((2, 2))
        self.sel_att = OCOR()
        self.c_down = nn.Conv2d(512, int(512 / 2), 1, bias=False)
        self.s_up = nn.Upsample(size=(7, 7))
    ...
    @auto_fp16()
    def forward(self, global_feat, roi_feat, proposal_feat):
        ...
        # refined by sos module
        sos_roi_feat = self.sos(roi_feat)

        # refined by ocor module
        num_obj_feat = int(sos_roi_feat.size()[0] / global_feat.size()[0])
        global_feat = self.ds_glb(global_feat)
        sos_roi_feat_ds = self.ds_obj(sos_roi_feat)
        bg_feat = global_feat.repeat((num_obj_feat, 1, 1, 1))
        obj_con_feats = torch.cat([sos_roi_feat_ds, bg_feat], dim=1)
        obj_con_obj_feats = self.sel_att(obj_con_feats)
        ocor_roi_feat = self.c_down(obj_con_obj_feats)
        ocor_roi_feat = self.s_up(ocor_roi_feat)

        # update roi feature
        roi_feat = ocor_roi_feat + sos_roi_feat
